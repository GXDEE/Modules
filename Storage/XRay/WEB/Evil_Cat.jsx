...
function App() {
    ...
    (async () => {
      try {
        ...
        if (!disposed && holder.current) {
          ...
          aura.className = "aura-lottie";
          holder.current.appendChild(aura);
          animation = window.lottie.loadAnimation({
            container: aura, renderer: "svg", loop: true, autoplay: true, animationData: data,
          });
        }
      } catch (_) {
  ...
  return React.createElement("main", { className: "cat-engine", onPointerDown: startSound },
    React.createElement("style", null, `
      .cat-engine{letter-spacing:.035em}
